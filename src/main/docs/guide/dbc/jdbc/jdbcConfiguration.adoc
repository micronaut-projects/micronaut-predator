...
Fields in given annotation `module` and `action` can set custom module and action. If omitted, then module value will default
...
being executed. There is also property `clientInfoAttributes` allowing to set arbitrary client info attributes to be set to the connection client info.
Annotation can be used on class or method, customizing module or action.
...
need to add this property `connection.customizer.oracleclientinfo.enabled=true`.
