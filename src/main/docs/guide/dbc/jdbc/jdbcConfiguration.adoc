...
=== JDBC Connection client info tracing

In order to trace SQL calls using `java.sql.Connection.setClientInfo(String, String)` method, you can
annotate a repository with the ann:data.jdbc.connection.annotation.ClientInfo[] annotation or ann:data.jdbc.connection.annotation.ClientInfo.Attribute[] for individual client info.

Note that the ann:data.jdbc.connection.annotation.ClientInfo.Attribute[] annotation can be used on either the class or the method, thus allowing customization of the  module or action individually.

For Oracle database, following attributes can be set to the JDBC connection client info: `OCSID.MODULE`, `OCSID.ACTION` and `OCSID.CLIENTID` and provided in ann:data.jdbc.connection.annotation.ClientInfo.Attribute[].
...
For other databases, new implementation of api:data.jdbc.connection.ConnectionCustomizer[] can be implemented.
