...
Note that if you supply the attributes `OCSID.MODULE` or `OCSID.ACTION` in the `clientInfoAttributes` member of the `@ConnectionClientInfo` annotation  these values will be ignored because
`module` and `action` attributes are already mapped to  `OCSID.MODULE` and `OCSID.ACTION`.
